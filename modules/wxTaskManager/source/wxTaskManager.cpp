...
wxTaskManager::wxTaskManager(std::shared_ptr<wxFTPWrapper> pclient, wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style, const wxString& name) : wxSplitterWindow(parent, id, pos, size, style, name), m_owner(pclient) {
	//
	//     notebook
	//
	wxNotebook* notebook;
	...
	notebook->SetMinSize(wxSize(700, 150));
	...
	//split
	SplitHorizontally(notebook, logCtrlPanel);
...
void wxTaskManager::TaskToVariant(const wxTransferTransaction& task, wxVector<wxVariant>& data) {
    ...
    #else
	    data.push_back(wxVariant(wxString::FromUTF8(task.local)));
    #endif
