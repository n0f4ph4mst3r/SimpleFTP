...
EVT_TREE_END_LABEL_EDIT(wxID_ANY, wxRemoteDirCtrl::OnEndEditItem)
...
wxRemoteDirCtrl::wxRemoteDirCtrl(std::shared_ptr<wxFTPWrapper> pclient, wxWindow* parent, wxWindowID id, const wxPoint& pos, const wxSize& size, long style) : wxPanel(parent, id, pos, size, style), m_owner(pclient) {
    m_tree = new wxTreeCtrl(this, wxID_ANY, wxDefaultPosition, wxDefaultSize, wxTR_HAS_BUTTONS | wxTR_HIDE_ROOT | wxTR_EDIT_LABELS);
    ...
    GetTreeCtrl()->Bind(wxEVT_LEFT_DOWN, &wxRemoteDirCtrl::MouseEvents, this);
    GetTreeCtrl()->Bind(wxEVT_RIGHT_DOWN, &wxRemoteDirCtrl::MouseEvents, this);
    GetTreeCtrl()->Bind(wxEVT_LEFT_DCLICK, &wxRemoteDirCtrl::MouseEvents, this);
    GetTreeCtrl()->Bind(wxEVT_RIGHT_DCLICK, &wxRemoteDirCtrl::MouseEvents, this);
    GetTreeCtrl()->Bind(wxEVT_KEY_DOWN, &wxRemoteDirCtrl::KeyDown, this);
...
void wxRemoteDirCtrl::CreateContext(wxRemoteTreeCtrlItemData* pdata) {
    ...
    if (!isFile) {
        ...
        Bind(wxEVT_COMMAND_MENU_SELECTED, &wxRemoteDirCtrl::OnPopupCreateDir, this, wxID_TREE_CREATE_DIR);
    }
...
void wxRemoteDirCtrl::OnBeginEditItem(wxTreeEvent& event) {
    ...
    if (parent == m_tree->GetRootItem()) event.Veto();
    else {
        wxRemoteDirData* pdata = dynamic_cast<wxRemoteDirData*>(m_tree->GetItemData(parent));
        if (!m_owner->SwitchDirectory(pdata->GetPath())) event.Veto();
    }
}
...
void wxRemoteDirCtrl::OnEndEditItem(wxTreeEvent& event) {
    ...
    if (RNFRresponse.ncode != 350) event.Veto();
...
void wxRemoteDirCtrl::OnPopupCreateDir(wxCommandEvent& event) {
    ...
    wxTextCtrl* pathCtrl = new wxTextCtrl(popupWindow, wxID_ANY, wxString::FromUTF8(defaultPath()), wxDefaultPosition, wxSize(300, -1));
    ...
    try {
        if (popupWindow->ShowModal() == wxID_OK) {
            ...
            #if defined(__WXMSW__)
                if (!CreateDir(wxString(path.utf8_str()).ToStdString(), rootId)) throw path;
            #else
                boost::locale::generator gen;
                std::stringstream ss;
                ss.imbue(gen("he_IL.UTF-8"));
                ss << path;
                if (!CreateDir(ss.str(), rootId)) throw path;
            #endif
        }
        else throw path;
            ...
            if (list->empty()) {
                ...
                while (!list->empty()) {
                    ...
                    if (type == "file") {
                        ...
                        #else
                           m_tree->SetItemImage(item, wxTheFileIconsTable->GetIconID(wxString::FromUTF8(itemData->GetExtension())));
                        #endif
                    }
                    else if (type == "dir") {
