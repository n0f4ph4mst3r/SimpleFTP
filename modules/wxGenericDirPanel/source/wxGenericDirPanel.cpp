...
void wxGenericDirPanel::LocalTreeOnDragInit(wxTreeEvent& event) {
    ...
    if (parent == m_clientCtrl->GetRootId()) event.Veto();
        ...
        if (dragResult != wxDragCopy || m_serverCtrl->WidgetBusy()) {
    ...
    if (parent == serverCtrl->GetRootItem()) event.Veto();
        ...
        if (dragResult != wxDragCopy) {
            ...
            std::string source = dynamic_cast<wxRemoteTreeCtrlItemData*>(serverCtrl->GetItemData(m_localDropTarget->GetSourceID()))->GetPath();
                ...
                ss << m_clientCtrl->GetPath(m_localDropTarget->GetTargetID());
...
wxThread::ExitCode wxGenericDirPanel::Entry() {
    if (m_bDirectionFlag) {
        ...
        std::string source_root = dynamic_cast<wxRemoteTreeCtrlItemData*>(m_serverCtrl->GetTreeCtrl()->GetItemData(m_localDropTarget->GetSourceID()))->GetPath();
            ...
            ss.imbue(gen("he_IL.UTF-8"));
            ss << m_clientCtrl->GetPath(m_localDropTarget->GetTargetID());
            std::string target_root = ss.str();

            if (!wxEndsWithPathSeparator(target_root)) target_root += wxFILE_SEP_PATH;
            ss.clear();
            ss.str(std::string());
            ss << target_root << wxString::FromUTF8(source_root).AfterLast('/');
            target_root = ss.str();
        #endif
        ...
        std::function<void(const wxTreeItemId&)> f_createTask = [&](const wxTreeItemId& item) {
            ...
            std::string target = pItemData->GetPath();
        ...
        if (ethread && ethread->IsRunning() && !GetThread()->TestDestroy()) ethread->Wait();
            ...
            std::stringstream ss;
            ss.imbue(gen("he_IL.UTF-8"));
            ss << m_remoteDropTarget->GetPath();
            std::string source_root = ss.str();
        #endif
        ...
        if (target_root == "") target_root += '/';
            ...
            ss.clear();
            ss.str(std::string());
            ss << target_root << wxFileName(wxString::FromUTF8(source_root)).GetFullName();
            target_root = ss.str();
        #endif
        ...
        std::function<void(const wxString&)> f_createTask = [&](const wxString& path) {
            ...
            if (wxFileName::FileExists(path)) {
                ...
                sstream >> size;
                    ...
                    ss.imbue(gen("he_IL.UTF-8"));
                    ss << path;
                    wxTransferTransaction task = wxTransferTransaction(ss.str(), target, true, size, false);
                #endif
                m_owner->AddTask(task);
                wxMilliSleep(500);
            }
                ...
                wxDir dir(path);
                    ...
                    ss.imbue(gen("he_IL.UTF-8"));
                    ss << path;
                    wxTransferTransaction task = wxTransferTransaction(ss.str(), target, false, 0, false);
                #endif
                m_owner->AddTask(task);
                wxMilliSleep(500);
        ...
        #else
            f_createTask(wxString::FromUTF8(source_root));
        #endif
    }
