...
ClientApp::ClientApp(const wxString& title)
    ...
    m_taskManager = new wxTaskManager(m_client, bottomPanel, wxID_ANY, wxDefaultPosition, wxSize(700, 300), wxSP_3D | wxSP_LIVE_UPDATE);
    ...
    topPanelSizer->Add(headerPanel, 0, wxEXPAND);
...
void ClientApp::ConnectionClicked(wxCommandEvent&) {
    ...
    if (portCtrl->GetValue() == wxEmptyString) m_accessData[wxID_CONNECTION_PORT] = "ftp";
    else {
        long port;
